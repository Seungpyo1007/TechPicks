...
## 세 번째 훑기 (2026-08-17) — 버튼이 글자 위에서만 눌렸다

코드에서만 보이는 자리를 팠더니, **앞의 두 번에서 겪고도 원인을 잘못 짚었던
것**이 나왔다. 온보딩 "다음"과 랭킹 찾기 버튼이 안 눌렸을 때 좌표 탓인 줄
알았는데, `GestureDetector` 의 기본값이 `deferToChild` 라 `decoration:` 으로
칠한 알약은 히트 테스트에 안 잡힌다. **글자 글리프 위에서만 눌렸다.**

빠져 있던 열 곳에 `HitTestBehavior.opaque` 를 넣었다. 새 테스트
(`test/widget/tap_edge_test.dart`)가 모서리에서 6pt 안쪽을 누른다 — 가운데만
누르던 기존 테스트는 글자를 맞혀 통과했다.

| 무엇 | 어떻게 드러났나 |
|---|---|
| 버튼이 글자 위에서만 눌림 | 시뮬레이터에서 두 번 겪고도 좌표 탓으로 넘겼다 |
| "내 기기" 가 iPhone 17 Pro 를 iPhone 17 이라 함 | 매처가 낱말이 아니라 부분 문자열로 셌다 |
| 저장값 복원이 방금 담은 기기를 지움 | 다섯 노티파이어가 복원을 안 기다렸다 |
| 상담 입력창이 키보드에 가림 | 하드웨어 키보드로 테스트해서 못 봤다 |
| 기다리는 동안 표시 없음·둘째 질문 증발 | `isBusy` 를 아무도 안 읽었다 |
| 홈 관심 목록이 기본 가중치로 지수를 그림 | 결론 카드와 한 화면에서 숫자가 달랐다 |
| "256GB 어때?" 가 예산 $256 | 아무 세 자리 숫자나 받았다 |
| 로그인 상태를 켠 순간에만 읽음 | `userChanges` 구독이 없었다 |
| 3D 뷰어 카피가 없는 기능 셋을 약속 | 스캔 화면과 같은 문제 |
| `compared` 이벤트 미호출 / `weight_changed` 폭주 | 비교 사용량이 영원히 0 |

`.github/workflows/flutter-ci.yml` 에 iOS 릴리스 빌드·골든 잡을 적어 뒀다.
**돌리지는 않았다** — 다 만들었다고 할 때 처음 돌린다.

이어서 실패 상태도 고쳤다 (`bdf0b79`). 카탈로그를 못 읽으면 다섯 화면이
각자 빈 상태로 떨어져 "관심 목록이 비어 있습니다"라고 말했다. 이제 공용
`TpErrorState` 로 못 읽었다고 말하고 다시 시도를 준다. 애셋 경로를 잠깐
틀어 시뮬레이터에서 눈으로 확인했다.
