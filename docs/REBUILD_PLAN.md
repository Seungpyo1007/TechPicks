...
- **Laptops 화면 (명세 §6)** — TechAPI `laptops` 는 1,951종인데 점수가 하나도
  ...
  칩은 그래서 눌리지 않는다.
- **가격 현지화** — 명세는 `$1,300` / `₩1,798,000` 처럼 언어별로 다른 값을
  쓰고 "환율로 환산하지 않는다"고 못박았다. TechAPI 에는 `msrp_usd` 밖에 없어
  한국어에서도 달러가 나온다. 원화 가격을 받거나 카탈로그에 같이 구워야 한다.
  You 화면의 Currency 줄이 `USD` 로 고정인 것도 같은 이유다.
