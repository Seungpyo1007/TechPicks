...
(() => {
  ...
  // utm_source identifies this project to Unsplash; keep it identical in
  // both places below (the homepage link and the referral normalizer).
  const UNSPLASH_HOMEPAGE_HREF =
    'https://unsplash.com/?utm_source=techpicks&utm_medium=referral';
  ...
  const withReferral = (href) => {
    try {
      ...
      if (!u.searchParams.has('utm_source')) {
        u.searchParams.set('utm_source', 'techpicks');
      }
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // Content. The sidecar is writable by external tooling, so its value
      // isn't guaranteed canvas-originated — only accept data:image/ URLs
      // from it. The `src` attribute is author-controlled, so it passes
      // through unchanged.
