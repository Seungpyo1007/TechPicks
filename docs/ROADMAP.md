...
| # | 무엇 | 왜 |
|---|---|---|
| P2.1 | `firebase_crashlytics` + 삼킨 15군데에 `recordError` | 실패가 프로덕션에서 안 보인다 |
| P2.2 | `firebase_analytics` — 가중치·비교·상담 이벤트 | 핵심 가설이 검증 안 됐다 |
| P2.3 | `riverpod_lint` + `custom_lint` | 26·27 회차 버그를 정적으로 잡는다 |
| P2.4 | `animations` 로 손으로 짠 shared axis 대체 | 90줄 삭제, 공식 구현 |

P2.1 이 가장 값이 크다. **P1 보다 먼저 해도 된다** — 출시 전에 넣어야 첫날부터
보인다.

P2.3·P2.4 는 각각 한두 시간이다.

### P3 — 제품 기능 (1–2주, 추정)

| # | 무엇 | 먼저 정할 것 |
|---|---|---|
| P3.1 | `share_plus` + `app_links` | 공유 문구, URI 스킴 |
| P3.2 | `firebase_remote_config` 로 카탈로그 전달 | 통째로 vs URL만 |
| P3.3 | `connectivity_plus` 오프라인 안내 | 문구 |
| P3.4 | `alchemist` 골든 테스트 | 유리 크롬 골든이 안정적인지 |
| P3.5 | `cloud_firestore` 관심 목록 동기화 | 로컬 우선 + 로그인 시 병합 설계 |

**P3.2 가 특히 값이 크다.** 명세가 "스토어 배포 없이 점수 갱신"을 목표로
적었는데 지금 카탈로그가 애셋이라 달성 못 하고 있다.
...
| 유리 크롬 골든이 환경마다 달라짐 | 중 | P3.4 를 시도해보고 안 되면 뺀다 |
...
지금 ────────────────────────────────────────────────
  P0  마감 대응            반나절   검증 완료
  P1.5 v1 권한 잔재 정리   1시간    지금 가능
  P2.3 riverpod_lint       1시간
  P2.4 animations 교체     2시간
  P2.1 Crashlytics         반나절
  P2.2 Analytics           반나절
...
그다음 ──────────────────────────────────────────────
  P3.2 Remote Config       2일      명세 목표 달성
  P3.1 공유 + 딥링크       2–3일
  P3.4 골든 테스트         2일
  P3.3 오프라인 안내       반나절
  P3.5 Firestore 동기화    3일

데이터가 오면 ──────────────────────────────────────
  P4  막힌 것들
```

**지금 당장 사람 없이 할 수 있는 건 P0 + P1.5 + P2 다.** 대략 이틀치이고
전부 외부 의존이 없다.
