...
조사하다 나왔다. 내 정보의 **비밀번호 변경·프로필 수정이 아무 동작도 안
하고 있었다.** TabHost 가 콜백을 안 넘겨서 눌러도 조용했다.

| 무엇 | 어떻게 |
|---|---|
| 비밀번호 변경 | Firebase 재설정 메일. 보낼 곳이 없는 계정에는 줄을 안 보여준다 |
| 프로필 수정 | 이름만 바꾸는 다이얼로그 하나 |

안내는 본문에 한 줄로 붙였다. **이 앱은 Scaffold 를 안 써서 SnackBar 가
못 뜬다** — TpShell 이 크롬을 직접 그린다. 로그인 화면도 같은 방식이다.

### P3.6 — 카탈로그를 실데이터로 (완료)

폰 10 → **154**, CPU 5 → 40, SoC 5 → 30, 브랜드 16. 애셋 41KB → 488KB.

`tool/build_catalog.dart` 가 목록 인덱스에서 후보를 뽑아 상세를 받아보고
거른다 — 정규 레코드(`base_model_slug` 가 null) · 점수 있음 · 최근 3년.
200 을 목표로 했지만 세 조건을 다 통과하는 것이 154종이었다.

```bash
dart tool/build_catalog.dart
```

랭킹 화면은 **상위 50까지만** 그린다(`RankScreen.maxRows`). 목록이
`Stack` + `AnimatedPositioned` 라 모든 행을 한 번에 만들고, 가중치를 바꾸면
행마다 막대가 동시에 트윈된다. 카탈로그 전체는 비교·픽커·상세·이번 주
변동이 그대로 쓴다.

상한을 유지할지 재봤다. 가중치를 60번 연속으로 바꾸며(슬라이더를 끄는 상황)
프레임당 시간을 잰 값이다. 맥에서 잰 것이라 절대값보다 기울기를 본다.
...
기본 오버헤드가 15ms 남짓이니 **행당 0.25ms** 씩 붙는다. 기기는 위젯 작업이
서너 배 느리므로 154행이면 슬라이더를 끄는 동안 프레임 예산(16ms)을 크게
넘긴다. 50행은 여유가 있다 — **상한을 유지한다.**

목록을 가상화하면서 순위 이동 애니메이션을 지키려면 구조를 다시 짜야 한다.
그때 상한을 올린다.

상세에는 브랜드 카드가 붙었다 — 국가·설립연도·로케일별 설명·웹사이트.

비교 선택 시트에는 **검색**이 붙었다. 154종을 손으로 굴려 찾을 수는 없다.
이름과 브랜드로 거른다 — 화면에 브랜드가 안 보여도 사람은 "samsung" 으로
찾는다. 명세에 없는 요소지만 명세의 카탈로그도 10종이었다.

### P4 — 밖에서 뭔가 와야 시작

2026-08-14 에 TechAPI 를 다시 받아 확인했다. 카탈로그는 채웠고, 나머지 둘은
왜 아직 막혔는지가 더 분명해졌다.

| 무엇 | 기다리는 것 |
|---|---|
| Laptops 화면 | 점수가 없고, **무게와 가격을 둘 다 가진 노트북이 표본 120대 중 0대**다. 출처 데이터셋이 가격만 주는 쪽(`abhinavflac`)과 무게만 주는 쪽(`Ammok`)으로 갈려 있어 명세 §6 의 4칸 그리드를 못 채운다 |
| 제품 사진 | `image_url` 이 `cdn.jsdelivr.net/gh/GetTechAPI/images/...` 를 가리키지만 그 저장소가 아직 없다 (오늘도 404) |
| 가격 현지화 | 원화 가격 필드 |
| 다크 모드 | 디자인 토큰 한 벌 |
| 검색 | 서버 쿼리. 카탈로그가 154종이 되면서 로컬 검색도 쓸모가 생겼다 |
| 알림 | 서버가 순위를 계산해 푸시 |
...
Gemini 를 앞에 두고 실패하면 로컬이 받도록 바꿨더니 (`ec233bc`) 실기에서
이유가 찍혔다:
