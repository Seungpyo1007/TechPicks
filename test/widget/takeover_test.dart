...
/// 인수 화면(스캔·3D 뷰어)은 화면을 통째로 쓴다.
///
/// 셸이 아래 안전 영역만큼 비워 두던 때, 어두운 스캔 화면 밑에 밝은 배경이
/// 띠처럼 남았다. 시뮬레이터에서 눈에 띄었다 — 넉넉한 테스트 캔버스에서는
/// 안전 영역이 0 이라 안 보인다.
void main() {
  setUp(initLocalization);

  Future<void> pumpTakeover(
    WidgetTester tester,
    Widget screen,
    TpChrome chrome,
  ) async {
    ...
    await pumpScreenNoSettle(tester, screen, chrome: chrome, size: _frame);
    await tester.pump(const Duration(milliseconds: 300));
  }

  for (final chrome in TpChrome.values) {
    testWidgets('$chrome: 스캔 화면이 아래 끝까지 어둡다', (tester) async {
      await pumpTakeover(tester, const ScanScreen(), chrome);

      final dark = tester.getRect(
        find.byWidgetPredicate(
          (w) => w is ColoredBox && w.color == ScanScreen.background,
        ),
      );
      expect(dark.top, 0);
